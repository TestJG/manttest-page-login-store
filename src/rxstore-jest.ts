      ...
      describe(givenDescription, () => {
        describe(whenDescription, () => {
          it(expectationsDescription, () => {
            ...
            const prom = actions
              .concat(Observable.of<Action>(StoreActions.finish.create()))
              .do(store.dispatch)
...
export const testLastStates =
  <TState, TStore extends Store<TState>>(
    ...
    createStore: () => TStore) => (
      ...
      const { timeout = 300, count = 1 } = options || {};
      describe(givenDescription, () => {
        describe(whenDescription, () => {
          it(expectationsDescription, () => {
            const store = createStore();
            ...
            const prom = actions
              ...
              .switchMap(() => Observable.empty<TState>())
              .concat(store.state$)
              .takeLast(count).toArray().timeout(timeout)
              .toPromise() as PromiseLike<TState[]>;
            return prom.then(expectations);
          });
        });    // Given a store 
